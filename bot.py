...
TOKEN = '6835646872:AAHsbYYu-EtING-APZwVWJgiCJS2pD54m7Q'  # your bot token here
...
def get_ipinfo_data(ip):
    api_url = f"https://ipinfo.io/{ip}?token=1ede0a58686315"
...
def get_ipgeolocation_data(ip):
    api_key = "ba7a6e25a9484a68916e9e22e5328547"
...
def handle_message(msg):
    ...
    if not is_user_authorized(user_id):
        bot.sendMessage(chat_id, f'You dont have plan. Contact @DidanXx for buying plan.\n\nYour ID: <code>{user_id}</code>', parse_mode='HTML')
        return
...
- /listblacklist - <strong>See all BlackList.</strong>'''
            ...
            message = '<strong>Owner</strong>: @DidanXx\n<strong>Version</strong>: 2.0\nIf you want to buy source. Contact @DidanXx'
            bot.sendMessage(chat_id, message, parse_mode='HTML')
            ...
            if not is_user_authorized(user_id):
                bot.sendMessage(chat_id, 'You dont have plan. Contact @DidanXx for buying plan.')
                return
            ...
            keyboard = ReplyKeyboardMarkup(
                keyboard=[
                    [KeyboardButton(text='TLS'), KeyboardButton(text='MIX'), KeyboardButton(text='BYPASS'), KeyboardButton(text='RAW')],
                    [KeyboardButton(text='❌ Cancel')],
                ],
                resize_keyboard=True,
                one_time_keyboard=True
          ...
          states[chat_id] = user_state
            ...
            if not user_state.target:
              ...
              user_attack_slots = info.get('attack_slots', 0)
              if concurrents <= 0:
                bot.sendMessage(chat_id, 'Your concurrents count is zero, Contact @DantalionC2 To Buy Plan')
                del states[chat_id]  
                return
        ...
        elif text == 'BYPASS' or text == 'TLS' or text == 'RAW' or text == 'MIX':
            ...
            user_state = UserState(method=method)
            states[chat_id] = user_state
        elif chat_id in states and states[chat_id].method in ['BYPASS', 'TLS', 'RAW', 'MIX']:
            ...
            if not user_state.target:
              ...
              user_attack_slots = info.get('attack_slots', 0)
              if concurrents <= 0:
                bot.sendMessage(chat_id, 'Your concurrents count is zero. Contact @DidanXx To Buy Plan')
                del states[chat_id]  
                return
              if user_attack_slots >= concurrents:
                bot.sendMessage(chat_id, 'MAX CONCURRENTS. Your concurrents limit has been reached. Please wait for an available slot.')
...
def check_expired_users():
    ...
    for user_id, user_info in list(authorized_users.items()):
        ...
        if expiry_date < now:
            del authorized_users[user_id]
            write_authorized_users(authorized_users)
            bot.sendMessage(user_id, 'Your plan has expired. Contact @DidanXx to renew.')
...
if __name__ == '__main__':
    ...
    admin_users = set()
    admin_ids = [5130319904, 1792592129]  # admin user id here
